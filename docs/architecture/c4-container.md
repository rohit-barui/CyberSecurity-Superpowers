...
| **6 Skill Scripts** | Individual `run.sh` for threat-model, secure-coding, static-analysis, pentest, ir | Bash, SKILL.md metadata |
...
| **Test Suite** | Bash-based tests validating each skill and the orchestrator | Bash |

## Relationships

The orchestrator acts as the entry point. The developer (via AI harness) invokes it, which then dispatches to one or more skills. Each skill reads its own config from `tools/` and may invoke external security tools via subprocess. The hook system is triggered automatically by Git lifecycle events. Supply-chain security (SBOM) is invoked separately via `scripts/generate-sbom.sh`.
...
C4Container
  ...
  System_Boundary(csb, "Cybersecurity Superpowers") {
    ...
    Container(threat, "Threat-Model Skill", "Bash + SKILL.md", "STRIDE-based threat modeling with CVSS scoring")
    Container(seccode, "Secure-Coding Skill", "Bash + SKILL.md", "OWASP secure coding checklists & guidance")
    ...
    Container(tests, "Test Suite", "Bash", "Unit tests for each skill")
  }
...
| Bash for testing | Lightweight, no language runtime required beyond bash itself |
...
3. **Run the playbook generator**:
   ```bash
   ./skills/cybersecurity/incident-response/run.sh --incident-type <type> [--output-dir <path>] [--dry-run]
   ```

4. **Create an incident response playbook** that includes:
...
5. **Document the playbook** in a file named `INCIDENT_RESPONSE_PLAYBOOK.md` in the project root (or update if exists) that includes:

...
2. **Run the appropriate tools**:
   ...
   - Use the `run.sh` script to orchestrate all tools:
     ```
     ./run.sh --target-dir . --output-dir artifacts/reports --dry-run --format md
     ```

3. **Collect and normalize results** from all tools into a common format.

4. **Generate a report** in `artifacts/reports/SECURITY_SCAN.md` that includes:
...
- `artifacts/reports/SECURITY_SCAN.md`: Contains the static analysis and dependency scan results with remediation steps.
- If the project already has a `SECURITY.md`, consider updating it with any new security requirements discovered.

## Examples

See `skills/cybersecurity/static-analysis/templates/` for the report template.
